...
def extract_indicators_from_telemetry(case_id: str, tel: Dict[str, Any], sender: str, origin_ip: str) -> List[Dict[str, str]]:
    ...
    for url in urls[:5]:  # Bound top 5 URLs to avoid visual noise
...
def build_threat_graph() -> Dict[str, Any]:
    """
    Builds a NetworkX graph across all ingested cases and returns
    ...
    for row in rows:
        ...
        risk = row["risk_score"]
        status = row["status"]
        tel = json.loads(row["telemetry"]) if row["telemetry"] else {}
        ...
        # Extract and connect indicators
    ...
    # Detect connected components (Campaign clusters)
    subgraphs = [G.subgraph(c).copy() for c in nx.connected_components(G)]
    
    campaigns = []
    for idx, sg in enumerate(subgraphs):
        # Extract cases in this cluster
        ...
        # We classify as a campaign if >= 2 cases share infrastructure, or a high-risk isolated incident
        avg_risk = sum(cases_meta[c]["risk_score"] for c in case_nodes) / max(len(case_nodes), 1)
        
        # Identify pivot infrastructure
        ...
        campaigns.append({
            ...
            "cases": [cases_meta[c] | {"case_id": c} for c in case_nodes],
    ...
    # Sort campaigns: clusters first, then highest risk
